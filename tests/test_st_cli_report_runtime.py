...
class CliReportSystemTest(unittest.TestCase):
    def test_cli_driven_agent_mimic_generates_complete_playback_report(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertGreaterEqual(len(report["timeline"]["lanes"]), 1)
            self.assertTrue(any(lane["lane_id"] == "process:primary" for lane in report["timeline"]["lanes"]))
            primary_lane = next(lane for lane in report["timeline"]["lanes"] if lane["lane_id"] == "process:primary")
            self.assertTrue(any(segment["state"] == "understand" for segment in primary_lane["state_segments"]))
            self.assertTrue(any(segment["state"] == "review" for segment in primary_lane["state_segments"]))
            self.assertTrue(any(marker["event_type"] == "transition_applied" for marker in report["timeline"]["event_markers"]))
            self.assertIn("workflow", report["timeline"])
