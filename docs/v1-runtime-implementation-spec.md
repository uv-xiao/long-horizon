...
1. **Install-and-run happy path**
   ...
   - keep `slides.html`/`slides-data.json` only as compatibility aliases when
     present, not as a second visualization;
...
- generates `reports/agent-brief.md`, `progress.md`, `progress.html`, and
  `report-data.json`;
- may generate `slides.html` and `slides-data.json` only as compatibility
  aliases to the canonical timeline report;
...
- `reports/slides.html` as a compatibility redirect/alias to `progress.html`
  when needed by older links
- `reports/slides-data.json` as compatibility metadata when needed by older
  tooling
...
- `slides.html` is not a separate report surface in v1. If generated, it should
  redirect or link to `progress.html` and state that the timeline report is the
  canonical view.
- `slides-data.json` is not a second data model. If generated, it should contain
  only compatibility metadata and/or the canonical timeline reference.
...
- serve `slides.html`/`slides-data.json` only as compatibility aliases when
  requested;
