...
1. **Install-and-run happy path**
   ...
   - assert `slides.html` and `slides-data.json` are not generated;
   - assert report data contains stable ids, event sequence, process/workflow
     state, anchors, Perfetto-like timeline lanes, state segments, event
     markers, inter-process message links, and human-readable report surfaces.
...
3. **Parent-child/observer/comment case**
   ...
   - generate report;
   - assert timeline/report data contains parent-child relation, communication
     or intervention message link, comment event, and no observer mutation of
     task board.
...
- If an implementation problem appears, exploit practical alternatives to make
  the end-to-end mechanism work instead of stopping at scaffolding.
...
- The final verification must report test commands, results, and the commit
  pushed.
...
- `STATUS.md`: feature ledger covering implemented v1 mechanisms and evidence.
...
- Cross-ledger ordering is ledger-local; reporter derives display order from
  causal refs, per-ledger seq, and timestamps.
...
- one canonical Perfetto-like timeline in `progress.html`;
- horizontal event-sequence axis with optional wall-clock labels;
- lanes for every task process, observer process, human channel, and system
  channel;
- process timeline bars segmented by workflow state/status so the located state
  is visible on the lane itself;
- message bars/links between lanes for durable cross-process events: spawn,
  steer, ack, artifact import, merge, policy update, review request/result,
  handoff, and human comment routing;
- event markers on their owning lanes, clickable to unfold details without
  leaving the timeline;
- state segments clickable to show a workflow state-transition diagram for the
  selected process and selected point in time;
- inspector for selected event/process/message/artifact/annotation;
- compact observer intervention and human comment markers in the main timeline;
- dedicated intervention/comment summaries below or beside the timeline;
- stable anchors for visible entities.

Timeline semantics:

- primary ordering axis is unified event sequence, not wall-clock time;
- wall-clock is displayed and filterable when present, but never used as the
  correctness order;
- each state segment covers an inclusive event-index range where the process is
  projected to the same workflow state/status;
- selecting a state segment renders the workflow transition graph with the
  selected state highlighted and allowed next transitions emphasized;
- selecting an event or message unfolds source payload, process refs, anchors,
  and provenance;
...
-> timeline lanes, state segments, event markers, and message links
-> report-data.json
-> progress.html timeline UI
```
...
- `slides.html` and `slides-data.json` are not generated in current v1.
- Old links should be handled outside the runtime report generator if a target
  repository needs migration compatibility.
...
- do not generate or serve slide compatibility artifacts;
...
Large artifact compression is implemented as a retention sidecar that preserves
source lineage with compressed copies and a manifest.

## Memory And Learned Adapters

V1 includes learned adapter and memory deposition support. If a missing adapter
is learned during a run, it can be used immediately, logged under run artifacts
for review, and promoted through the promotion runtime into reusable skills,
rules, memory, or adapter templates with evidence and rollback records.
...
11. Local report server, real git worktree child spawning/import, and GitHub
    adapter smoke path.
12. Docs, status evidence, changelog, final verification.
...
8. `progress.html` shows a human-readable Perfetto-like execution timeline with
   process lanes, workflow state bars, event markers, inter-process message
   links, selected-state workflow diagram, and inspector data.
...
13. Tests pass and `STATUS.md` maps every v1 feature to implementation and
